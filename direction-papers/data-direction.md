...
The current AGI white paper's three flagships and the ANNOTATE R01's three datasets are all naturalistic-stimulus datasets. Yet the corpus contains six cognitive-task batteries plus three single-stimulus localizers with combined sample size greater than 16,000 subjects and zero community-curated HED Generation 3 sidecars across the entire group. The two sub-categories follow the [`dataset-hierarchy`](../research/synthesis/dataset-hierarchy.md) taxonomy.
