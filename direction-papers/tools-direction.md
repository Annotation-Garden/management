...
This paper supports the thesis through a structured survey of the existing FAIR services across the five layers of the Phase 1 [`tool-ontology`](../research/synthesis/tool-ontology.md), names the missing adapters, and sketches a three-wave build order. Section 2 walks the inheritance accounting; Section 3 names the missing middleware; Section 4 makes the composition argument; Section 5 sequences the build; Section 6 discusses; Section 7 concludes.
...
AGI does not duplicate any of these. The Garden architecture is hub-and-spoke: each stimulus becomes a versioned repository that NEMAR, OpenNeuro, NeuroScout, and DANDI index by reference. DataLad provides the cross-cutting provenance substrate. NeuroScout is the closest existing precedent for AGI's analysis side, and a NeuroScout-AGI bridge in which AGI annotations become NeuroScout predictors is the obvious near-term collaboration.
...
- ELAN `.eaf` to `events.tsv`: tier hierarchies become nested HED tags or sidecar JSON. The [`elan` card](../research/collection/tools/elan/card.md) names this adapter as a precondition for capturing existing linguistics and infant-cognition investments.
- Praat TextGrid to `events.tsv`: phonemes, words, prosody become separate annotation types. Whisper plus Praat is the natural pairing for transcription seed plus phonetic refinement; see the [`praat` card](../research/collection/tools/praat/card.md).
...
AGI does not own any of the upstream tools or any of the downstream platforms. It owns the contract between them. The contract is what makes a community-versioned annotation layer possible at all; without it every annotation effort produces a per-paper artifact and the branches-as-perspectives model in [`VISION.md`](../VISION.md) cannot operate. The composition argument is therefore institutional rather than software-engineering: AGI is the body best placed to host the contract that no single tool maintainer is incentivized to write.
...
This paper surveys the existing FAIR services and names the missing adapters; it does not implement them. Per-adapter technical specifications belong in a forthcoming `tooling/` directory. The composition argument also assumes that maintainers of the upstream tools (ELAN, Praat, BORIS, Datavyu, Pliers) will accept AGI as the canonical bridge to BIDS. The assumption is plausible because the bridge is bidirectional and does not require upstream-tool changes, but it is not pre-negotiated.

A second limitation is that the survey reflects the Phase 1 corpus's collection scope. Tools used heavily in adjacent communities (CVAT, Prodigy, web-based crowdsourcing platforms beyond Label Studio) are not in the corpus and not in the inheritance accounting. The omission does not invalidate the missing-middleware argument; it does mean those tools will need adapter contracts this paper does not specify.

A third limitation is that the AI-annotator landscape moves fast. The adapter contract is designed to be model-agnostic (Pliers extractor pattern plus HED tag emission), but each specific model adapter has a finite half-life as the foundation-model frontier advances.

### 6.3 Future work

The empirical question is whether labs that adopt AGI adapters subsequently publish more annotated-stimulus encoding analyses than they would otherwise, controlling for funding and lab size. Quantifying the lift requires baseline measurement of the per-paper annotation cost, an adoption-tracking mechanism for AGI repositories, and a comparable lab-level outcome measure. None of these exist today; they are reasonable Phase 4-or-later AGI deliverables. Complementary work extends the inheritance accounting beyond the Phase 1 corpus to CVAT, Prodigy, additional BIDS Apps, and the rapidly evolving foundation-model adapter ecosystem; the Phase 2 [`tool-ontology`](../research/synthesis/tool-ontology.md) is the natural document to extend.

### 6.4 Anticipated objection

A reader could argue that AGI is taking on too much: integrating five Layer 1 tools, eight Layer 2 AI annotators, and a CrowdAgent-pattern quality-control layer is operationally infeasible for a community-governed project. The response is that the per-adapter cost is small (a few hundred lines plus a HED-mapping table), the cumulative cost is amortized across the community that would otherwise pay it per paper, and the build order in Section 5 compounds the cost: each wave delivers a working example and the adapter pattern that the next wave reuses.
